...
uint16_t correctValue(uint16_t value, uint8_t humidity) {
...
int calculate_index_(uint16_t value, uint16_t array[AMOUNT_OF_LEVELS][2]) {
...
uint16_t AQICalculator::getAqi(uint16_t pm2_5_value, uint16_t pm10_0_value, uint8_t humidity)  {
    int pm2_5_index = calculate_index_(correctValue(pm2_5_value,humidity) , pm2_5_calculation_grid_);
    int pm10_0_index = calculate_index_(correctValue(pm10_0_value,humidity), pm10_0_calculation_grid_);
